...
class SyntaxLogWidget(QTextEdit):
    """Thread-safe scrolling log with Pygments syntax highlighting."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.setFont(_MONO)
        self.setStyleSheet("background:#0d0d1a; color:#e0e0e0; border:1px solid #2a2a4e;")
...
class SettingsWindow(QDialog):
    ...
    _settings_cls: ClassVar[Optional["Settings"]]    = None
    ...
    def _bottom_bar(self) -> QHBoxLayout:
        bar = QHBoxLayout()
        self._btn_export = QPushButton("📦 Export Config")
        ...
        self._btn_import.clicked.connect(self._import_cfg)
        self._btn_save.clicked.connect(self._save)
        return bar

    def _tab_models(self) -> QWidget:
        w, v = QWidget(), QVBoxLayout()
        w.setLayout(v)
        ...
        self._asr_combo.addItems(["large-v3 (local INT8)", "medium (local)", "openai/whisper-1 (API)"])
        ...
        self._llm_backend.addItems(["OpenAI API", "Local llama.cpp", "Ollama", "Groq", "Together AI"])
        ...
        f.addRow("Model / filename:",  self._llm_model)
        f.addRow("API Key:",           self._llm_key)
        f.addRow("Base URL:",          self._llm_url)
        f.addRow("Local model path:",  self._llm_path)
        v.addWidget(g)
        ...
        self._tts_backend.addItems(["openai", "huggingface", "piper"])
        self._tts_voice    = QLineEdit(self._s.tts_voice)
        self._tts_hf_repo  = QLineEdit(self._s.tts_hf_repo)
        self._tts_hf_token = QLineEdit(self._s.hf_token)
        self._tts_hf_token.setEchoMode(QLineEdit.EchoMode.Password)
        f.addRow("Backend:",        self._tts_backend)
        f.addRow("Voice / model:",  self._tts_voice)
        f.addRow("HF repo ID:",     self._tts_hf_repo)
        f.addRow("HF token:",       self._tts_hf_token)
        v.addWidget(g)
    ...
    def _tab_system(self) -> QWidget:
        ...
        self._clip_check    = QCheckBox("Enable clipboard integration (wl-clipboard)")
    ...
    def _tab_plugins(self) -> QWidget:
        ...
        btn = QPushButton("🔄  Reload Plugins")
        btn.clicked.connect(self._reload_plugins)
        v.addWidget(btn)
        return w
    ...
    async def _on_log(self, payload: EventPayload) -> None:
        ...
        if hasattr(self, "log_widget"):
            # Thread-safe: SyntaxLogWidget._html_signal handles it
    ...
    def _save(self) -> None:
        ...
        self._s.hf_token                 = self._tts_hf_token.text()
        self._s.save()
    ...
    def _export(self) -> None:
        ...
        if path:
            ...
            if ok:
                ...
                self.log_widget.append_log("success", f"Config exported → {path}")
    ...
    def _import_cfg(self) -> None:
        ...
        if path:
            ...
            if ok:
                ...
                self.log_widget.append_log("success", f"Config imported ← {path}")
    ...
    def _reload_plugins(self) -> None:
        ...
        self._plugin_list.setPlainText(
            "\n".join(loader.plugin_names()) or "(no plugins found)"
        )
